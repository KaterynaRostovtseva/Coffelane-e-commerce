...
export default function ProductCardPage() {
  ...
  if (!selectedProduct)
    return <Box sx={{ py: 10, textAlign: "center" }}>Product not found</Box>;

  const recommended = items
    .filter((p) => String(p.id) !== String(id))
    .slice(0, 3);
  console.log("Full product data:", selectedProduct);
  return (
    <Box sx={{ width: "100%", pb: 8 }}>
      ...
      >
        ...
        >
          <ProductImageSlider
            photos={
              selectedProduct.photos_url?.length > 0
                ? selectedProduct.photos_url
                : selectedProduct.product_photos?.length > 0
