...
export default function OrderEdit() {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        if (!orderData) {
          const listResponse = await apiWithAuth.get("/orders/admin-list/", {
            params: { page: 1, size: 100 },
          });
          const allOrders =
            listResponse.data?.data || listResponse.data?.results || [];
  ...
  useEffect(() => {
    if (!order?.positions?.length) return;
    ...
    const loadPhotos = async () => {
      ...
      order.positions.forEach((pos) => {
        ...
        if (item?.id && !photoCache.has(`${type}-${item.id}`)) {
          if (!itemsToFetch.find((i) => i.id === item.id && i.type === type)) {
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      <Typography sx={{ ...h3, mb: 3 }}>
        Edit Order #{order?.id || id}
      </Typography>
      ...
      {order && (
        ...
        >
          <Stack spacing={3}>
            <Box>
              <Typography
                sx={{ ...h5, mb: 3, fontWeight: 600, color: "#3E3027" }}
              >
                Order Information
              </Typography>
              ...
              {order.positions && order.positions.length > 0 && (
                <Box>
                  ...
                  >
                    <Typography sx={{ ...h7, color: "#666" }}>
                      Order Items ({order.positions.length})
                    </Typography>
                    <Box
                      sx={{
                        display: "flex",
                  ...
                  <Stack spacing={1.5} sx={{ mt: 1 }}>
                    {order.positions.map((position, idx) => {
                      const itemData =
                        position.product || position.accessory || {};
                      const itemName =
                        itemData.name ||
                        position.product?.name ||
                        position.accessory?.name ||
                        "Unknown item";
                      ...
                      if (!photoUrl) {
                        ...
                        if (
                          itemData.photos_url &&
                          Array.isArray(itemData.photos_url) &&
                          itemData.photos_url.length > 0
                        ) {
                          ...
                          itemData.product_photos &&
                          Array.isArray(itemData.product_photos) &&
                          itemData.product_photos.length > 0
                        ) {
                          const firstPhoto = itemData.product_photos[0];
                          photoUrl =
                            firstPhoto?.url || firstPhoto?.photo || null;
                        } else if (
                          itemData.accessory_photos &&
                          Array.isArray(itemData.accessory_photos) &&
                          itemData.accessory_photos.length > 0
                        ) {
                      ...
                      return (
                        ...
                        >
                          {hasImage ? (
                            ...
                            >
                              <CoffeeIcon
                                sx={{ fontSize: 30, color: "#ccc" }}
                              />
                            </Box>
                          )}
                          <Box sx={{ flex: 1, minWidth: 0 }}>
                            <Typography
                              sx={{ ...h6, mb: 0.5, fontSize: "0.95rem" }}
                            >
                              {itemName}
                            </Typography>
                            <Box
            ...
            <Box>
              <Typography sx={{ ...h5, mb: 2, fontWeight: 600 }}>
                Status
              </Typography>
